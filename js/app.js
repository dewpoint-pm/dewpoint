...
function setTipoPerfume(tipo, btn){
  ...
  if(btnD) btnD.style.cssText=btnD.style.cssText.replace(/background:[^;]+;color:[^;]+;border-color:[^;]+/,'')+(tipo==='decants'?onStyle:offStyle);
  if(btnB) btnB.style.cssText=btnB.style.cssText.replace(/background:[^;]+;color:[^;]+;border-color:[^;]+/,'')+(tipo==='botella'?onStyle:offStyle);
...
function guardarPerfume(){
  ...
  var tipo=(document.getElementById('mp-tipo')||{}).value||'decants';
  /* Para botellas usar los campos específicos */
  ...
  if(APP._editPerfumeId){
    ...
    DB.crearPerfume(datos, function(ok,msg){
      if(ok){
        showToast('Perfume creado ('+unidades+' unidad'+(unidades>1?'es':'')+')');
        cerrarModal('modal-perfume');
        delete _lastLoad['perfumes'];
        loadPerfumes(); loadPerfumesVenta();
      } else showToast('Error: '+(msg||'No se pudo crear'));
