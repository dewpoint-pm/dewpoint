...
function actualizarComboPerfumes(){
  ...
  /* FIX: solo decants o solo botella, sin parcial */
...
function abrirEditarPerfume(id){
  ...
  if(p.tipo_venta==='botella'&&p.ml_totales>0){
...
function renderGrafico(){
  ...
  function _draw(datosV, datosC){
    ...
    if(!periodos.length){
      ...
      ctx.fillStyle=tc.label; ctx.font='13px sans-serif'; ctx.textAlign='center';
      ctx.fillText('Sin datos para el per\u00edodo', W/2, H/2); return;
    }
    var vMap={}, cMap={};
    ...
    var areaW=W-PAD_L-PAD_R, areaH=H-PAD_T-PAD_B;
    var n=periodos.length, nSeries=serie==='ambos'?2:1;
    ...
    function drawBar(ix,offset,val,colorMid,colorTop){
      ...
      if(altura>6){
        ...
        ctx.lineTo(bx,byTop+4); ctx.quadraticCurveTo(bx,byTop,bx+4,byTop);
    ...
    for(var i2=0;i2<periodos.length;i2++){
      ...
      var gx2=PAD_L+gap+i2*(groupW+gap), cx=gx2+groupW/2;
      var p=periodos[i2], etq=p;
    ...
    if(legEl){
      ...
      var legTxt='';
      if(serie==='ambos'||serie==='ventas') legTxt+='<span style="display:flex;align-items:center;gap:5px"><span style="width:14px;height:14px;background:#2E8B57;border-radius:3px;display:inline-block"></span>Ventas</span>';
      if(serie==='ambos'||serie==='costos') legTxt+='<span style="display:flex;align-items:center;gap:5px"><span style="width:14px;height:14px;background:#C0392B;border-radius:3px;display:inline-block"></span>Costos</span>';
...
function validarRut(rutClean){
  ...
  var body=rutClean.slice(0,-1), dv=rutClean.slice(-1);
  var suma=0, mult=2;
...
function setTipoPerfume(tipo,btn){
  var tipoEl=document.getElementById('mp-tipo');
...
function guardarPerfume(){
  ...
  var datos={
    ...
    tipo_venta:tipo,unidades:unidades,
...
function showToast(msg){
  ...
  void t.offsetHeight;
  t.style.transition=''; t.classList.add('show');
...
document.addEventListener('DOMContentLoaded',function(){
  DB.onSessionExpired(function(){
    ...
    setTimeout(function(){
      ...
      var lu=document.getElementById('lu'); var lp=document.getElementById('lp'); var le=document.getElementById('le');
      if(lu)lu.value=''; if(lp)lp.value='';
      if(le){le.textContent='Tu sesi\u00f3n expir\u00f3. Inicia sesi\u00f3n de nuevo.';le.style.display='block';}
    },1500);
  });
  var ls=document.getElementById('login-screen'); if(ls)ls.style.display='flex';
  var selP=document.getElementById('sel-perfume'); var selF=document.getElementById('sel-formato');
  if(selP) selP.addEventListener('change',onPerfumeSel);
  if(selF) selF.addEventListener('change',onFormatoSel);
});
